define(["loading", "dialogHelper", "formDialogStyle", "emby-checkbox", "emby-select", "emby-input", "alert"],
    function (loading, dialogHelper, alert) {
        ...
        function openIpDialog(view) {
            ...
            html += '<input type="text" id="ipAddress" name="threshold" placeholder="Ip Address" />';
            html += '</div>';
            html += '</div>';
        ...
        function openPersonalAccessTokenDialog() {
            var dlg = dialogHelper.createDialog({
                size: "medium-tall",
                removeOnClose: !0,
                scrollY: !1
            ...
            dlg.classList.add("ui-body-a");
            dlg.classList.add("background-theme-a");
            dlg.classList.add("smartThings");
            dlg.style = "max-width:50%;";

            var html = "";
            html += '<div class="formDialogHeader" style="display:flex">';
            html += '<button is="paper-icon-button-light" class="btnCloseDialog autoSize paper-icon-button-light" tabindex="-1"><i class="md-icon"></i></button><h3 class="formDialogHeaderTitle">Personal Access Token</h3>';
            html += '</div>';

            html += '<div class="formDialogContent" style="margin:2em; height:35em">';
            html += '<div class="dialogContentInner dialog-content-centered">';
            html += '<div class="inputContainer">';
            html += '<svg version="1.1" x="0px" y="0px" viewBox="0 0 578.2 94.1" xml:space="preserve">';
            html += '<use xlink:href="#newLogo" />';
            html += '</svg>';
            html += '<h3 id="welcomeMessage" style="text-align: center; font-size: x-large;">Welcome to Samsung Smart Things integration with Emby Media Server</h3>';
            ...
            html += '<p>If you don\'t have it already, they can be created and managed on the <a href="https://account.smartthings.com/login?redirect=https%3A%2F%2Faccount.smartthings.com%2Ftokens" target="_blank">personal access tokens page</a></p>';

            html += 'Next, go to the personal access tokens page and click on the Generate new token on top-right.</p>';
            ...
            ApiClient.getPluginConfiguration(pluginId).then((config) => {
                if (config.PersonalAccessToken) {
                    dlg.querySelector('#personalAccessToken').value = config.PersonalAccessToken;
                }
            });
        ...
        function openDeviceEditorDialog(device, app, config, view) {
            ...
            html += '<input type="time" id="scheduleTime" label="Schedule Time to trigger events" class="emby-input">';
            ...
            ApiClient.getJSON(ApiClient.getUrl("GetSmartThingsScenes")).then(
                (result) => {

                    if (result.items) {
                        
                        result.items.forEach((scene) => {

                            moviePlaybackStartedSelect.innerHTML   += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            moviePlaybackStoppedSelect.innerHTML   += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            moviePlaybackPausedSelect.innerHTML    += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            moviePlaybackUnPausedSelect.innerHTML  += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            seriesPlaybackStartedSelect.innerHTML  += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            seriesPlaybackStoppedSelect.innerHTML  += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            seriesPlaybackPausedSelect.innerHTML   += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            seriesPlaybackUnPausedSelect.innerHTML += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                            liveTvPlaybackStartedSelect.innerHTML  += ('<option value="' + scene.sceneId + '">' + scene.sceneName + '</option>');
                        ...
                        if (config.SaveSmartThingsProfiles) {
                            config.SaveSmartThingsProfiles.forEach(option => {
            ...
            dlg.querySelector('#saveButton').addEventListener('click',
                () => {
                    ...
                    ApiClient.getPluginConfiguration(pluginId).then((config) => {
                        ...
                        config.SaveSmartThingsProfiles = listSavedOptions;
            ...
            view.addEventListener('viewshow',
                () => {
                    ...
                    */
                    ...
                    view.querySelector('#personalAccessTokenButton').addEventListener('click', () => {
                        openPersonalAccessTokenDialog();
                    });
                });
        };
